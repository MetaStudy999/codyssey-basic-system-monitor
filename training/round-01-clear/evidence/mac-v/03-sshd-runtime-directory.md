...
- `sshd -t`: **PASS**
- effective SSH configuration baseline: **PASS / captured**
- current SSH port: **22 (baseline only)**
- current `PermitRootLogin`: **without-password (must change to `no`)**
- Ubuntu 24.04 socket activation: **CONFIRMED**
- UFW baseline: **inactive**

## 상태 경계

이 Evidence는 SSH 설정 변경 전의 Runtime Directory 복구와 baseline만 증명합니다.
